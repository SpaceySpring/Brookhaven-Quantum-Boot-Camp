...
"""
Q1.Why does the power spectrum of the sampled waveform have so many frequencies?
Answer_1. Power spectrum of a wave gives the distribution of the signal power among various frequency and its the Fourier transform of the function.
In this we see the Power spectrum resembling the Fourier graph as expected. The reason why the power spectrum of the sampled waveform have so many frequencies is
its expected for the wave to be infinite. This is because sampling in time domain is the same from multiplying with an infinite set of impulses

Q2.Why does the magnitude of the DC component =1⁄2 ?
Answer_2. The wave is produced by a voltage of 1/2 making it not a pure AC waveform because the DC is not zero. The DC Voltage and the DC component (The constant voltage added) are the same.

Q3.Why are there no sine components in its DFT?
Answer_3. On the signal graph we see the wave being even which only gives us real integers making the cosine appear. If there was imaginary numbers then they would have cancelled out.
"""
